...
export default function BenefitsGridSection({
  ...
  return (
    <section className="py-12 text-white">
      <div
        className={`mx-auto grid w-[calc(100%-40px)] max-w-[1560px] items-stretch gap-3 sm:w-[calc(100%-64px)] md:grid-cols-2 xl:grid-cols-[minmax(0,1.25fr)_repeat(3,minmax(0,1fr))] ${
  hasSecondBenefitRow ? "xl:grid-rows-2" : ""
      ...
      >
        ...
        <motion.div
          ...
          className={`flex min-h-80 min-w-0 flex-col justify-center overflow-hidden bg-[#b34b0c] p-7 sm:min-h-96 xl:h-full xl:px-9 xl:py-8 ${
  hasSecondBenefitRow ? "xl:row-span-2" : ""
}`}
        >
          <motion.h2
            variants={headingContentVariants}
            className="max-w-full font-['Outfit'] text-[clamp(2.25rem,2.2vw,2.625rem)] font-bold leading-[1.15]"
          >
            {heading}
          </motion.h2>
