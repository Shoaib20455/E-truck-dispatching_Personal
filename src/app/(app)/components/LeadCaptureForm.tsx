...
function FormField({
  ...
  return (
    <motion.label variants={itemVariants} className="block text-[0.6875rem] font-semibold">
      {label}
      <input
        name={name}
        type={type}
...
export default function LeadCaptureForm({
  ...
  const { status, message, handleSubmit, isSending } = useEmailForm(source);
  ...
  return (
    <motion.form
      ...
      className={`relative z-30 mx-4 mb-8 space-y-4 bg-[#191919] p-4 shadow-2xl ${className}`}
    >
      <motion.span
        variants={{
          hidden: { scaleX: 0 },
