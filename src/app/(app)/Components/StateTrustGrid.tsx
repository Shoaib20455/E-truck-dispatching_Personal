      ...
      <div className="mx-auto max-w-[1520px] px-6 lg:px-8 2xl:px-0">
        ...
        <StaggerGroup stagger={0.08} className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => (
            <StaggerItem key={item.title} preset={index % 2 === 0 ? "tile-left" : "tile-right"} hover="premium" className={`rounded-[20px] p-7 ${item.highlighted ? "border border-teal-500 bg-teal-500/10 shadow-[0px_10px_20px_rgba(0,0,0,0.07)]" : "bg-white"}`}>
